...
from feat.emo_detectors.StatLearning.EmoSL_test import EmoSVMClassifier, EmoRandomForestClassifier
...
if __name__ == "__main__":
    ...
    # emo_regressor = EmoSVMClassifier()  # 속도는 빠른데 emotion별로 실수값으로 나옴
    emo_regressor = EmoRandomForestClassifier()  # 속도는 느린데 one-hot 형식으로 나옴
    ...
    fer_session = onnxruntime.InferenceSession('onnx/FER.onnx', None)  # emotion 검출용
    ...
    while cap.isOpened():
        ...
        if (boxes.shape[0] == 0):
            ...
            for i in range(boxes.shape[0]):
                ...
                vers = face_utils.recon_vers2D([param], [[box[0], box[1], box[2], box[3]]], u_base, w_shp_base, w_exp_base)[0]  # 내 코드
                vers = np.array(vers)


                # ===========================피부 영역 정의 후 밝기값 추출=========================================
                img = orig_image.copy()  # 이미지 복사, BGR 이미지
                mask_img = np.zeros((480, 640))  # 피부영역 정의용 mask, 입력 영상 크기에 맞춰야 함
                # mask_img = np.zeros((1024, 1280))
                ...
                skin = cv2.bitwise_and(gray, gray, mask=mask_img)  # 정의한 피부 영역, (480, 640, 3) 또는 (1024, 1280, 3)
                ...
                # print("AU shape: ", au_occur.shape)
                print("AU: ", f"{au_occur[0]:.2f}, {au_occur[1]:.2f}, {au_occur[3] * 10:.2f}, {au_occur[17]:.2f}")
                # au4 = [au_occur[0], au_occur[1], au_occur[3], au_occur[17]]
                # au4 = np.array(au4)
                # au_outputs = au4 * 100

                # ====================================== Detect facial expressions ===================================
                # ANGER, DISGUST, FEAR, HAPPINESS, SADNESS, SURPRISE, NEUTRAL
                emo_occur = emo_regressor.detect_emo(frame=hogs, landmarks=new_lands)  # 작동함
                print("emotion: ", f"{emo_occur[0][0]:.2f}, {emo_occur[0][1]:.2f}, {emo_occur[0][2]:.2f}, {emo_occur[0][3]:.2f}, {emo_occur[0][4]:.2f}, {emo_occur[0][5]:.2f}, {emo_occur[0][6]:.2f}")
                ...
                # ================================== csv 파일 저장 ==================================
                f = open('csv/sg_ex.csv', 'a', newline='')
                wr = csv.writer(f)
                wr.writerow([mean, round(au_occur[0], 3), round(au_occur[1], 3), round(au_occur[3], 3), round(au_occur[17], 3),
                             round(emo_occur[0][0], 3), round(emo_occur[0][1], 3), round(emo_occur[0][2], 3), round(emo_occur[0][3], 3),
                             round(emo_occur[0][4], 3), round(emo_occur[0][5], 3), round(emo_occur[0][6], 3)])
                f.close()
                print("csv 저장")
