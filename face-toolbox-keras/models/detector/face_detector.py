...
class FaceAlignmentDetector(BaseFaceDetector):
    ...
    # 랜드마크 1개만 그리기
    def draw_landmark(image, landmarks, color=(0, 255, 0), stroke=3):
        x, y = landmarks
        image = cv2.circle(image.copy(), (int(y), int(x)), stroke, color, -1)        
        return image

    # 지정한 인덱스 범위 내의 랜드마크 여러개 그리기
    def draw_landmark_range(image, landmarks, start, end, color=(0, 255, 0), stroke=3):
